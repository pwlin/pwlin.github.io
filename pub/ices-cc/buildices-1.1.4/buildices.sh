...
# Copyright 2007-2018, Centova Technologies Inc.
#
...
function verify_exists {
	...
	if [ -z "$TESTFILE" ]; then
		...
		echo "     yum install gcc gcc-c++ make pkgconfig"
		echo "For Debian based systems, use:"
		echo "     apt-get install gcc g++ make pkg-config"
		echo ""
		exit 1
	fi
}
...
verify_exists pkg-config "pkg-config"
...
for f in $FILES; do
	...
	case "$f" in
		...
		"libshout.tar.gz")
			...
			echo ""
			if [ $STATICBUILD -gt 0 ]; then
				LDFLAGS="-static" ./configure --prefix=$INSTPREFIX --with-ogg=$INSTPREFIX --with-vorbis=$INSTPREFIX -with-pic --enable-shared --enable-static
			else
				./configure --prefix=$INSTPREFIX --with-ogg=$INSTPREFIX --with-vorbis=$INSTPREFIX
			fi
			[ $? -gt 0 ] && echo "Configure failed for $f; aborting" && exit 1
			echo ""
		...
		"ices.tar.gz")
			...
			if [ $STATICBUILD -gt 0 ]; then
				PKG_CONFIG_PATH=${INSTPREFIX}lib/pkgconfig ./configure --prefix=$INSTPREFIX --with-xml-config=${INSTPREFIX}bin/xml2-config $FEATUREOPT $LAMEOPT --without-perl --without-python --without-flac -with-pic --disable-shared --enable-static
			else
				PKG_CONFIG_PATH=${INSTPREFIX}lib/pkgconfig ./configure --prefix=$INSTPREFIX --with-xml-config=${INSTPREFIX}bin/xml2-config $FEATUREOPT $LAMEOPT --without-perl --without-python --without-flac
			fi
			[ $? -gt 0 ] && echo "Configure failed for $f; aborting" && exit 1
			echo ""
			echo "Building ices ..."
